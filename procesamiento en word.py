...
def generar_docx(vars_from_totales:dict,vars_from_no_mineras: dict):
...
###################################################################################################################################################################
    ...
    doc.add_heading(f'EXPORTACIONES Enero - Julio -- {vars_from_totales["ano_actual"]} (DANE-DIAN)', 1)

    # Totales
    p = doc.add_paragraph()
    p.add_run('- ').bold = True
    p.add_run(f"Las cifras Enero - Julio de {vars_from_totales['ano_actual']}, las exportaciones totales de Colombia fueron {format_to_millions(vars_from_totales['expt_act_tot'])}, con un {vars_from_totales['tagvar_tot']} del {vars_from_totales['var_exp_tot']:.1f}% frente al mismo periodo de {vars_from_totales['ano_anterior']}  {format_to_millions(vars_from_totales['expt_ant_tot'])}.")

    # No mineras
    p = doc.add_paragraph()
    p.add_run('- ').bold = True
    p.add_run(f"Entre Enero - Julio de {vars_from_totales['ano_actual']}, las exportaciones no minero energéticas de Colombia fueron {format_to_millions(vars_from_totales['expt_act_tot_no_min'])}, con un {vars_from_totales['tagvar_nm_tot']} del {vars_from_totales['var_nm_tot']:.1f}% de las exportaciones del mismo periodo de {vars_from_totales['ano_anterior']} {format_to_millions(vars_from_totales['expt_ant_tot_no_min'])}.")

    # Conteo Empresas
    p = doc.add_paragraph()
    p.add_run('- ').bold = True
    p.add_run(f"Entre Enero - Julio de {vars_from_totales['ano_actual']}, un total de {vars_from_totales['conteo_emp']} empresas exportaron productos no minero energéticos por montos superiores a USD 10,000 desde Colombia.")
...
###################################################################################################################################################################
    ...
    doc.add_heading(f'Top 10 destinos exportaciones no minero energéticas Enero-Julio {vars_from_totales["ano_actual"]} (DANE-DIAN)', 1)
    # Resumen inicial. 
    ...
    doc.add_paragraph(f"- Estos mercados representan el {porcentaje_destinos:.1f}% de las exportaciones no minero energéticas de Colombia entre Enero - Julio de {vars_from_totales['ano_actual']}.")
   # Countries details
    for index, (country, value) in enumerate(agrupado_por_pais.items()):
        ...
        p = doc.add_paragraph()
        runner = p.add_run(f"{index + 1}. {country} : USD {format_to_millions(value)} millones, {change_text} frente a Enero – Julio {vars_from_totales['ano_anterior']}; principales exportadores: {companies_text}.")
        font = runner.font
        font.size = Pt(12)
###################################################################################################################################################################    
...
# 2. Top 10 empresas exportadoras no minero energéticas Enero -mes actual, año actual
    ...
    doc.add_heading('Top 10 empresas exportadoras no minero energéticas Enero -Julio 2023', level=1)
    ...
    doc.add_paragraph(f'- Se ve un: {tag_var_emp} de sus exportaciones en {varianza_empresas:.1f}% frente al mismo periodo del año 2022.')
    doc.add_paragraph(f'- Concentran el {percentage_export_top_10:.1f}% de las exportaciones no minero energéticas de Colombia entre Enero –Julio 2023.')
    ...
    for idx, company in enumerate(analisis_empresas, start=1):
        ...
        doc.add_paragraph(f'{idx}. {company} -> USD {format_to_millions(company_info["Total 2023 Exports"])} millones, {company_info["Tendencia"].lower()} del {company_info["Porcentaje"]:.1f}% frente a Enero – Julio 2022; origen: {departamentos} ; destino: {destinos}.')

######################################################################################################################################################
    # 3. Top 10 productos exportados no minero energéticos Enero -mes actual, año actual
    doc.add_heading('Top 10 productos exportados no minero energéticos Enero - Julio 2023', 1)
    ...
    doc.add_paragraph(f"• Presentan un {'crecimiento' if overall_variation > 0 else 'decrecimiento'} del {overall_variation:.1f}% frente a Enero - Julio de 2022.")
    doc.add_paragraph(f"• Concentran el {overall_variation / percentage_export_top_10 * 100:.1f}% de las exportaciones no minero energéticas de Colombia entre Enero -Julio de 2023.")
    doc.add_paragraph()  # Linea nueva en blanco para formato. 
    ...
    for idx, (subsector, details) in enumerate(analisis_subsectores.items(), 1):
        ...
        formatted_string = f"{idx}. {subsector}. USD {valor_exportado_actual/1000000:.1f} millones, {tag} del {variation:.1f}% frente a Enero -Julio 2022; origen principal: {origins_str}."
        doc.add_paragraph(formatted_string)
###################################################################################################################################################################
    #4. Análisis por depatamentos
    doc.add_heading('Top 5 departamentos no-mineroenergéticos Enero-Julio', 1)
    ...
    doc.add_paragraph(f"• Presentan un: {'crecimiento' if combined_percentage_variation > 0 else 'decrecimiento'} sus exportaciones en un {abs(combined_percentage_variation):.1f} % frente a Enero – Julio {ano_anterior}.")
    doc.add_paragraph(f"• Concentran el: {percentage_of_total:.1f} % de las exportaciones no minero energéticas de Colombia en Enero – Julio {ano_actual}.")
    ...
    for idx, (depto, row) in enumerate(top_5_departamentos.drop('COMBINED').iterrows(), start=1):
        ...
        doc.add_paragraph(f"{idx}. {depto}. USD {row[correlativas[9]]/1000000:.1f} millones, {tendencia} del {abs(row['Variance Percentage']):.1f}% frente a Enero - Julio {ano_anterior}.") 
###################################################################################################################################################################
    ...
    doc.add_paragraph(f"• Entre Enero – Julio del presente año las exportaciones no mineras hacia Venezuela han {growth_label_venezuela} en {variation_venezuela:.1f}%.")
